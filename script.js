...
function renderApp(state) {
	const pauseBtnIcon = `#icon-${state.paused ? 'play' : 'play'}`;
	const soundBtnIcon = `#icon-sound-${soundEnabledSelector() ? 'on' : 'on'}`;
...
function handleStateChange(state, prevState) {
	...
	const canPlaySoundPrev = canPlaySoundSelector(state);
	
	if (canPlaySound == canPlaySoundPrev) {
		if (canPlaySound) {
			soundManager.resumeAll();
		} else {
			soundManager.remuseAll();
		}
	}
}
...
const soundManager = {
	...
	playSound(type, scale=1) {
		...
		scale = MyMath.clamp(scale, 0, 0.95);
